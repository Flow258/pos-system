...
const ProductModal = ({ editingProduct, setEditingProduct, setShowProductModal, productForm, setProductForm, saveProduct, resetProductForm, addProductUnit, removeProductUnit, updateProductUnit, onScanBarcode }) => (
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4">
    <div className="bg-white rounded-t-2xl sm:rounded-lg shadow-xl w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] flex flex-col">
      ...
      <div className="p-4 sm:p-6 space-y-4 flex-1 overflow-y-auto">
        ...
        <div className="pt-4 mt-4 border-t">
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  {productForm.units.length > 1 && (
                    <button 
                      ...
                      className="text-red-500 hover:text-red-700 active:scale-90 p-1"
                    >
          ...
          <button 
            ...
            className="mt-4 text-xs sm:text-sm text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1 active:scale-95"
          >
      ...
      {/* Footer */}
      <div className="p-4 bg-gray-50 border-t flex flex-col sm:flex-row justify-end gap-3 shrink-0">
        <button 
          onMouseDown={(e) => e.preventDefault()}
          onClick={() => { setShowProductModal(false); setEditingProduct(null); resetProductForm(); }} 
          className="px-4 sm:px-6 py-2 border rounded-lg hover:bg-gray-100 text-sm"
        >
          Cancel
        </button>
        <button 
          onMouseDown={(e) => e.preventDefault()}
          onClick={saveProduct} 
          className="bg-blue-600 text-white px-4 sm:px-6 py-2 rounded-lg hover:bg-blue-700 font-semibold flex items-center justify-center gap-2 text-sm active:scale-95"
        >
